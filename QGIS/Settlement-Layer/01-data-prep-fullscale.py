#OBS: Ensure that all input data has spatial indexes. Improves performance significantly.
...
# Load data
sett = gpd.read_file(r'input\pplp-sample.shp')
admin = gpd.read_file(r'input\admin.shp')
pop = rio.open(r'input\pop-sample.tif', masked=True)


### Adding hard boundaries to raster
# Turn rows in geodataframe into list of linestrings
lines = []
for x in range(len(admin.index)):

    geometry = admin.iloc[x].geometry    #gets a polygon from each row (i.e. object) in geodataframe
...
admin_lines = gpd.read_file(r'process\admin_lines.shp').buffer(100)
